    ...
    if accepting!=[]:
        for key in edges:
            for value in edges[key]: 
                 if acceptedPath(value,edges,accepting,[]) != None:
                     if key not in ed:
                         ed[key]=[]
                     ed[key]=ed[key]+[value]
        for x in accepting:
...
def acceptedPath(current,edges,accepting,visited):
    if current in accepting:
        return ""
    for node in edges:
        if current in node and node not in visited:
           #print "start",current,visited
           visited.append(node)
           temp=None
           for nextState in edges[node]:
               if nextState in accepting:
                    #print "jump"
                    return ""  
               for y in edges:
                   if nextState in y and y not in visited:
                        #print "nextstate",current,nextState
...
#print new_edges4 == { (1, 'a'): [1],(1, 'b'): [2], (2, 'b'): [3], (3, 'c'): [2, 1], }
